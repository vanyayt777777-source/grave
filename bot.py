...
Исправлено:
- Работа кнопок оплаты картой/СБП
...
class Database:
        ...
        async with self.pool.acquire() as conn:
            ...
            for cat, data in config.VERIFICATION_PRICES.items():
                await conn.execute("""
                    ...
                    VALUES ('verification', $1, $2, $3, $4, $5, $6)
                ...
                """, cat, data["name"], data["price_rub"], data["price_usd"], data["price_coin"], data["available"])
            ...
            for cat, data in config.EDUCATION_PRODUCTS.items():
                await conn.execute("""
                    ...
                    VALUES ('education', $1, $2, $3, $4)
                    ON CONFLICT (type, category) DO UPDATE SET
                        title = EXCLUDED.title,
                        price_rub = EXCLUDED.price_rub,
                        available = EXCLUDED.available
                """, cat, data["name"], data["price"], data["available"])
    ...
    async def delete_account(self, account_id: int, reason: str = None):
        """Удаление аккаунта"""
        async with self.pool.acquire() as conn:
            await conn.execute(
...
class Keyboards:
    ...
    def account_selection(accounts):
        ...
        for acc in accounts:
            # Дешифруем номер для отображения (только последние 4 цифры)
            try:
                phone = encryption.decrypt(acc['phone_number'])
                display_phone = f"...{phone[-4:]}" if len(phone) > 4 else phone
            except:
                display_phone = "???"
            
            builder.button(
                text=f"{acc['title']} ({display_phone}) - {acc['price_rub']}₽",
    ...
    def admin_accounts_list(accounts):
        ...
        for acc in accounts[:10]:  # Показываем первые 10
            ...
            builder.button(
                text=f"{status} {acc['title']} - {acc['price_rub']}₽",
                ...
                async with session.get(f"{self.base_url}/getInvoices", headers=headers, params=params) as resp:
                    if resp.status == 200:
                        ...
                        if result.get("ok") and result.get("result"):
                            return result["result"].get("items", [])[0]
                    return None
        except Exception as e:
...
async def account_selected(callback: CallbackQuery, state: FSMContext):
    ...
    # Дешифруем номер для отображения
    try:
...
async def verifications_handler(message: Message):
    ...
    for prod in products:
        if prod['available']:
            text += f"{prod['title']} - {prod['price_rub']}₽ / {prod['price_usd']}$ / {prod['price_coin']}🪙\n"
...
async def verification_selected(callback: CallbackQuery, state: FSMContext):
    ...
    async with db.pool.acquire() as conn:
        product = await conn.fetchrow(
            "SELECT * FROM products WHERE type='verification' AND category=$1",
            category
        )
...
async def education_handler(message: Message):
    ...
    for prod in products:
        if prod['available']:
            text += f"{prod['title']} - {prod['price_rub']}₽\n"
...
async def education_selected(callback: CallbackQuery, state: FSMContext):
    ...
    async with db.pool.acquire() as conn:
        product = await conn.fetchrow(
            "SELECT * FROM products WHERE type='education' AND category=$1",
            category
        )
...
async def admin_verification_action(callback: CallbackQuery):
...
@dp.callback_query(lambda c: c.data.startswith("admin_verif_price_"))
async def admin_verification_price(callback: CallbackQuery, state: FSMContext):
    """Изменение цены верификации"""
...
@dp.callback_query(lambda c: c.data.startswith("admin_verif_toggle_"))
async def admin_verification_toggle(callback: CallbackQuery):
    """Включение/выключение верификации"""
    product_id = int(callback.data.split("_")[3])
    product = await db.get_product(product_id)
...
async def admin_education_action(callback: CallbackQuery):
...
@dp.callback_query(lambda c: c.data.startswith("admin_edu_price_"))
async def admin_education_price(callback: CallbackQuery, state: FSMContext):
    """Изменение цены обучения"""
    product_id = int(callback.data.split("_")[3])
    await state.update_data(product_id=product_id)
...
@dp.callback_query(lambda c: c.data.startswith("admin_edu_toggle_"))
async def admin_education_toggle(callback: CallbackQuery):
...
async def admin_account_action(callback: CallbackQuery, state: FSMContext):
    ...
    if parts[2] == "del":
        ...
        await callback.message.edit_text(
            "❓ Укажите причину удаления (или отправьте '-' если не нужна):",
            reply_markup=Keyboards.cancel_button()
        )
        await state.set_state(AdminStates.waiting_for_delete_account_reason)
        await callback.answer()
        return
...
@dp.message(AdminStates.waiting_for_delete_account_reason)
async def admin_delete_account(message: Message, state: FSMContext):
    ...
    await db.delete_account(account_id, reason)
...
async def admin_requisites(message: Message):
    ...
    if admin and admin["card_details"]:
        ...
        text += f"   Получатель: {card.get('name', 'Не указан')}\n"
    else:
        text += f"💳 Карта: ❌ Не указана\n"
    
    if admin and admin["sbp_details"]:
        sbp = admin["sbp_details"]
        text += f"📱 СБП: {sbp.get('phone', 'Не указан')}\n"
        text += f"   Банк: {sbp.get('bank', 'Не указан')}\n"
    else:
        ...
        if purchase:
            ...
            if account:
                ...
                try:
                    ...
                    await sms.client.start(bot_token=None, phone=lambda: account["phone_number"])
