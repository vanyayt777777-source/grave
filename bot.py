"""
Telegram бот "Grave Shop" - ФИНАЛЬНАЯ ВЕРСИЯ
- Исправлены команды одобрить/отклонить (правильные regexp фильтры)
- После верификации пишет @NeverByeq за выдачей
- Управление обучением работает
"""
...
class Config:
    ...
    # Контакт для выдачи верификаций
    VERIFICATION_CONTACT = "@NeverByeq"
...
class AdminStates(StatesGroup):
    ...
    waiting_for_check_decision = State()
...
# ----- ТЕКСТОВАЯ ОБРАБОТКА ЧЕКОВ АДМИНОМ (С ПРАВИЛЬНЫМИ FILTERS) -----

@dp.message(F.text.regexp(r'^одобрить\s+\d+$', re.IGNORECASE))
            ...
            if verif_purchase:
                ...
                if product["type"] == "verification":
                    await bot.send_message(
                        ...
                        f"Обратитесь к {config.VERIFICATION_CONTACT} за выдачей"
                    )
                elif product["type"] == "education":
                    # Отправляем обучающий материал
                    material = await db.get_education_material(product["id"])
...
@dp.message(F.text.regexp(r'^отклонить\s+\d+$', re.IGNORECASE))
