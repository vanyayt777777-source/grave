"""
Telegram бот "Grave Shop" - ИСПРАВЛЕННАЯ ВЕРСИЯ
- Исправлена ошибка с regexp в aiogram 3.x
- Команды одобрить/отклонить работают
"""
...
class AdminStates(StatesGroup):
    ...
    waiting_for_education_text = State()
    ...
    waiting_for_check_decision = State()  # Для текстового подтверждения чеков

class ReviewStates(StatesGroup):
    waiting_for_rating = State()
    waiting_for_comment = State()

# ==================== КАСТОМНЫЕ ФИЛЬТРЫ ДЛЯ КОМАНД ====================

class ApproveCheckFilter(BaseFilter):
    async def __call__(self, message: Message) -> bool:
        if not message.text:
            return False
        # Проверяем формат "одобрить 123" (регистронезависимо)
        pattern = r'^одобрить\s+(\d+)$'
        return bool(re.match(pattern, message.text.strip(), re.IGNORECASE))

class RejectCheckFilter(BaseFilter):
    async def __call__(self, message: Message) -> bool:
        if not message.text:
            return False
        # Проверяем формат "отклонить 123" (регистронезависимо)
        pattern = r'^отклонить\s+(\d+)$'
        return bool(re.match(pattern, message.text.strip(), re.IGNORECASE))
...
class Database:
    ...
    async def update_product_text(self, product_id: int, text: str):
        """Обновление текста продукта"""
        async with self.pool.acquire() as conn:
            await conn.execute("""
...
class Keyboards:
    ...
    def admin_education_actions(product_id: int):
        ...
        builder = InlineKeyboardBuilder()
        builder.button(text="📤 Загрузить файл", callback_data=f"admin_edu_upload_{product_id}")
        builder.button(text="📝 Изменить текст", callback_data=f"admin_edu_text_{product_id}")
        ...
        builder.button(text="👁 Просмотреть", callback_data=f"admin_edu_view_{product_id}")
...
class TelegramAuth:
    ...
    async def get_sms_code_from_chat(self, session_string: str):
        """Получение SMS кода из чата Telegram с использованием существующей сессии"""
        try:
...
async def receipt_handler(message: Message, state: FSMContext):
    ...
    await state.update_data(purchase_id=purchase_id, check_id=check_id)
...
# ----- ТЕКСТОВАЯ ОБРАБОТКА ЧЕКОВ АДМИНОМ (ИСПРАВЛЕНО С КАСТОМНЫМИ ФИЛЬТРАМИ) -----

@dp.message(ApproveCheckFilter())
async def approve_check_text(message: Message):
    """Подтверждение чека через текстовую команду"""
    ...
    # Извлекаем ID чека
    match = re.search(r'одобрить\s+(\d+)', message.text, re.IGNORECASE)
    if not match:
        return
    ...
    # Проверяем, что чек назначен этому админу
    if check["assigned_admin"] != message.from_user.id:
        await message.answer("❌ Этот чек не предназначен для вас")
        return
    
    # Получаем информацию о покупке
    async with db.pool.acquire() as conn:
        if check["account_id"]:
            # Это покупка аккаунта
            purchase = await conn.fetchrow(
                "SELECT * FROM purchases WHERE account_id = $1 AND status = 'pending' ORDER BY created_at DESC LIMIT 1",
                check["account_id"]
            )
            ...
            if purchase:
                ...
                await conn.execute("""
                    UPDATE purchases SET status = 'completed', processed_by = $1
                    WHERE id = $2
                """, message.from_user.id, purchase["id"])
                ...
                if account:
                    ...
                    await bot.send_message(
                        ...
                        "⭐️ Понравился аккаунт? Оставьте отзыв!",
                        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text="📝 Оставить отзыв", callback_data=f"review_{purchase['id']}")]
                        ])
                    )
        
        elif check["product_id"]:
            # Это покупка верификации/обучения
            verif_purchase = await conn.fetchrow(
                "SELECT * FROM verification_purchases WHERE product_id = $1 AND status = 'pending' ORDER BY created_at DESC LIMIT 1",
                check["product_id"]
            )
            ...
            if verif_purchase:
                await conn.execute("""
                    ...
                    WHERE id = $2
                """, message.from_user.id, verif_purchase["id"])
    ...
    await message.answer(f"✅ Чек {check_id} подтвержден")

@dp.message(RejectCheckFilter())
async def reject_check_text(message: Message):
    """Отклонение чека через текстовую команду"""
    # Проверяем, что пользователь админ
    user = await db.get_user(message.from_user.id)
    if not user or not user["is_admin"]:
        return
    ...
    # Извлекаем ID чека
    match = re.search(r'отклонить\s+(\d+)', message.text, re.IGNORECASE)
    if not match:
        return
    ...
    await message.answer(f"❌ Чек {check_id} отклонен")
...
async def education_selected(callback: CallbackQuery, state: FSMContext):
    ...
    # Сохраняем описание для отображения
...
async def admin_education_action(callback: CallbackQuery, state: FSMContext):
    ...
    if len(parts) >= 4:
        ...
        if action == "upload":
            ...
            await state.update_data(product_id=product_id)
            await callback.message.edit_text(
                f"📝 Отправьте новый текст описания для этого материала:"
            )
            await state.set_state(AdminStates.waiting_for_education_text)
            await callback.answer()
            return
            ...
            if material:
                ...
                text += f"📊 Статус: {'✅ Доступен' if product['available'] else '❌ Недоступен'}\n\n"
                text += f"📎 Файл: {material['file_name']}"
                ...
                await callback.message.edit_text(
                    f"📚 {product['title']}\n\n"
                    f"📝 Описание:\n{product['description'] if product['description'] else 'Нет описания'}\n\n"
                    f"💰 Цена: {product['price_rub']}₽\n"
...
@dp.message(AdminStates.waiting_for_education_text)
async def admin_education_text_set(message: Message, state: FSMContext):
    """Установка нового текста описания"""
    text = message.text.strip()
    
    data = await state.get_data()
    product_id = data.get("product_id")
    
    await db.update_product_text(product_id, text)
    
    await message.answer(
        f"✅ Текст описания обновлен!",
        reply_markup=Keyboards.admin_menu()
    )
    await state.clear()
...
# ----- ПОЛУЧЕНИЕ SMS КОДА ПОКУПАТЕЛЕМ -----
...
async def get_sms_code_handler(callback: CallbackQuery):
    ...
    # Получаем SMS код с использованием существующей сессии
    tg_auth = TelegramAuth(config.API_ID, config.API_HASH)
    code = await tg_auth.get_sms_code_from_chat(session_string)
