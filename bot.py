...
- Старые сессии работают (сохраняются в базе)
- У админа текстовые кнопки для подтверждения/отклонения
- У админа убрана кнопка получения кода (теперь только у покупателя)
- В управлении обучением появилось меню с действиями
"""
...
async def receipt_handler(message: Message, state: FSMContext):
    ...
    caption = (
        ...
        f"✅ approve_{check_id} - подтвердить\n"
        f"❌ reject_{check_id} - отклонить"
    )
...
# ----- ТЕКСТОВАЯ ОБРАБОТКА ЧЕКОВ АДМИНОМ -----
